...
- `engines` — `IEngine` (`SetPixel`, `DrawTexture`, `DrawTextureTiled`, `LoadTexture`/`UnloadTexture`, `GetApplicationDirectory`) + `EngineFactory::GetEngine()`, backed by `backends/raylib/RaylibEngine`. The seam through which the actual raylib draw calls happen.
...
Both factories also expose a test-only override hook — `SoundFactory::SetCreator()` and `EngineFactory::SetEngine()` — that let `tests/mock_sound.h`/`tests/mock_engine.h` substitute a `MockSound`/`MockEngine` in place of the real backend, with no real audio device or window/render context involved. `tests/test_soundmanager.cpp` and `tests/test_texturecanvas.cpp` are the first consumers of each seam; `TileMapRenderer`, `Sprite`, and `CollisionManager` could use the same `MockEngine` seam but don't have tests yet.
