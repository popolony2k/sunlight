...
- `filesystem` — `IFileSystem` (`Init`/`Shutdown`/`Mount`/`Exists`/`ReadFile`/`SetReadFilter`) + `FileSystemFactory::GetFileSystem()`, backed by `backends/physfs/PhysFsFileSystem`. Mounts a real, on-disk location (a loose directory or a real archive - PhysFS auto-detects which) so every resource read (textures, sound, tilemaps) goes through one virtual path space instead of the OS filesystem directly. Mounting itself is left to the caller - but if nothing was ever explicitly mounted, `PhysFsFileSystem` lazily mounts the process's current working directory at the virtual root the first time any read happens, reproducing plain `fopen()`'s old CWD-relative behavior so a consumer that never calls `Mount()` doesn't silently lose all resource loading. `SetReadFilter` registers an optional post-read transform (eg. transparent decryption) applied to every successful `ReadFile` result before it reaches the caller - unset by default, so `ReadFile` behaves exactly as before this hook existed; sunlight itself carries no opinion on what a registered filter does.
