...
- `window` — `IWindow` (`Create`/`Close`, `ShouldClose`, `SetExitKey`, `BeginFrame`/`EndFrame`, `SetFullscreen`/`GetFullscreen`, `SetWindowResizeable`, `SetTargetFPS`, `SetWindowTitle`, `GetScreenWidth`/`GetScreenHeight`, `GetElapsedTime`) + `WindowFactory::GetWindow()`, backed by `backends/raylib/RaylibWindow`. Everything about the OS window itself and the per-frame present cycle; split from `IEngine` so a window and the thing that draws into it are separate concerns.
