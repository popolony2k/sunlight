...
`sound/SoundFactory`/`ISound` follows the same shape, but `SoundFactory::CreateSound()` returns a fresh `std::unique_ptr<ISound>` on every call rather than a shared singleton reference like `EngineFactory::GetEngine()` — `SoundManager::Load()` needs a brand new instance per loaded sound id (see `SoundManager::m_SoundMap`), so a singleton wouldn't fit. This is also the seam a mock `ISound` would plug into to unit test `SoundManager`'s id-keyed dispatch logic without a real audio device.
